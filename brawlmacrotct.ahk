...
ParchesPaginas() {
    ...
    { ico: Chr(0x1F4CB), tit: "Parche v32.6",
      txt: "· Grafo x100: orbes con glow, estelas y minimapa`n"
         ...
         . "· Modo cine (tour solo) y Big Bang (espacio)`n"
         . "· Curiosidades: la más llamada, la más profunda,`n"
         . "   posibles funciones sin uso...`n"
         . "· Guardar PNG, pantalla completa, FX automáticos`n" },

    { ico: Chr(0x1F4CB), tit: "Parche v32.5.1",
      txt: "· 🕸 Grafo de llamadas: botón nuevo en Personalizar`n"
         . "· Mapa vivo de qué función llama a cuál, en el`n"
         . "   navegador y sin internet`n"
         . "· Pulsos animados viajando por las llamadas`n"
         . "· Clusters por área con etiqueta y color propios`n"
         . "· Top conexiones, modo enfoque (ver solo su red),`n"
         . "   zoom, buscador y filtro de flujo principal`n" },
...
; HTML autocontenido: canvas con física de fuerzas y render aditivo (orbes con
; glow, estelas, minimapa navegable), simulador de flujo en cascada, camino
; desde Iniciar, modo cine, big bang, insights del código, exportar PNG y
; calidad automática según los FPS reales de la máquina.
...
GrafoConstruirHtml(nodesJson, edgesJson, seedsJson) {
    ...
    h .= '<style>`n'
    h .= 'body{margin:0;background:#0b0b13;color:#eee;font:13px "Segoe UI",sans-serif;overflow:hidden}`n'
    h .= '#cv{display:block;cursor:grab}`n'
    h .= '#mm{position:fixed;right:12px;bottom:12px;border:1px solid #3a3a55;border-radius:8px;background:rgba(10,10,18,.85);cursor:pointer}`n'
    h .= '#ui{position:fixed;top:10px;left:10px;background:rgba(14,14,26,.92);border:1px solid #34344a;border-radius:12px;padding:10px 12px;width:258px;backdrop-filter:blur(4px);max-height:94vh;overflow:auto}`n'
    h .= '#info{position:fixed;top:10px;right:10px;width:295px;max-height:92vh;overflow:auto;background:rgba(14,14,26,.92);border:1px solid #34344a;border-radius:12px;padding:10px 12px;display:none;backdrop-filter:blur(4px)}`n'
    ...
    h .= '#titulo{background:linear-gradient(90deg,#4cd964,#4aa8ff,#b47aff,#ff6b6b);-webkit-background-clip:text;background-clip:text;color:transparent;font-weight:700;animation:hue 9s linear infinite;display:inline-block}`n'
    h .= '@keyframes hue{from{filter:hue-rotate(0deg)}to{filter:hue-rotate(360deg)}}`n'
    h .= '#tops a{display:flex;align-items:center;gap:6px}`n'
    h .= '#tops .dot{width:8px;height:8px}`n'
    h .= 'button{background:#1c1c30;border:1px solid #3a3a55;color:#dde;border-radius:6px;padding:3px 8px;cursor:pointer;font-size:11px;font-family:inherit}`n'
    h .= 'button:hover{background:#272740}`n'
    h .= 'button.on{background:#2c4a2c;border-color:#4cd964}`n'
    h .= 'details{margin-top:6px}summary{cursor:pointer;font-weight:600}`n'
    h .= '#insBody a{display:inline}`n'
    h .= '#insBody p{margin:5px 0}`n'
    h .= '</style></head><body>`n'
    h .= '<canvas id="cv"></canvas>`n'
    h .= '<canvas id="mm" width="170" height="120"></canvas>`n'
    h .= '<div id="ui"><b>' Chr(0x1F578) ' <span id="titulo">Grafo de llamadas</span></b><div class="mut" id="stats"></div>`n'
    h .= '<input id="q" type="text" placeholder="Buscar funcion y pulsar Enter">`n'
    h .= '<div style="display:flex;gap:5px;flex-wrap:wrap;margin:4px 0">`n'
    h .= '<button id="bSim">Simular flujo</button><button id="bCine">Modo cine</button><button id="bBang">Big bang</button>`n'
    h .= '<button id="bPng">Guardar PNG</button><button id="bFull">Pantalla completa</button><button id="bFx">FX: Auto</button></div>`n'
    h .= '<label><input id="flujo" type="checkbox"> Solo flujo principal</label>`n'
    h .= '<div id="leyenda"></div>`n'
    h .= '<div style="margin-top:6px"><b>Top conexiones</b></div><div id="tops"></div>`n'
    h .= '<details><summary>Curiosidades del codigo</summary><div id="insBody" class="mut"></div></details>`n'
    h .= '<div class="mut" style="margin-top:6px">Rueda: zoom - Arrastrar: mover - Clic nodo: detalles - Espacio: big bang - Esc: soltar todo</div></div>`n'
    h .= '<div id="info"></div>`n'
    h .= '<script>`n'
    ...
    h .= 'const CATANG={nucleo:0,ciclo:0.9,perfiles:1.8,webhook:2.7,stats:3.6,ui:4.5,otros:5.4};`n'
    h .= 'const N=NODES.length;const IN=[],OUT=[];const EIDX=new Map();`n'
    h .= 'for(let i=0;i<N;i++){IN.push([]);OUT.push([]);}`n'
    h .= 'EDGES.forEach((e,ei)=>{OUT[e[0]].push(e[1]);IN[e[1]].push(e[0]);EIDX.set(e[0]+"-"+e[1],ei);});`n'
    ...
    h .= 'const ROOT=NODES.findIndex(n=>n.n==="Iniciar");`n'
    h .= 'const DEPTH=new Array(N).fill(1e9);`n'
    h .= 'if(ROOT>=0){DEPTH[ROOT]=0;const q=[ROOT];let qi=0;while(qi<q.length){const v=q[qi++];for(const w of OUT[v])if(DEPTH[w]>DEPTH[v]+1){DEPTH[w]=DEPTH[v]+1;q.push(w);}}}`n'
    h .= 'function reachCount(r){const vis=new Set([r]);const st=[r];while(st.length){const v=st.pop();for(const w of OUT[v])if(!vis.has(w)){vis.add(w);st.push(w);}}return vis.size-1;}`n'
    h .= 'const RCNT=new Array(N);for(let i=0;i<N;i++)RCNT[i]=reachCount(i);`n'
    h .= 'const catOn={};for(const k in CATS)catOn[k]=true;`n'
    h .= 'let soloFlujo=false,focusI=-1;const visible=new Array(N).fill(true);`n'
    h .= 'const cv=document.getElementById("cv"),ctx=cv.getContext("2d");`n'
    h .= 'const mm=document.getElementById("mm"),mtx=mm.getContext("2d");`n'
    h .= 'const CR=300;const CCENT={};for(const k in CATS)CCENT[k]={x:Math.cos(CATANG[k])*CR,y:Math.sin(CATANG[k])*CR};`n'
    h .= 'const P=NODES.map(n=>{const a=CATANG[n.c]+(Math.random()-0.5)*0.8,r=CR*(0.5+Math.random()*0.6);return{x:Math.cos(a)*r,y:Math.sin(a)*r,vx:0,vy:0};});`n'
    h .= 'const ST1=[],ST2=[];`n'
    h .= 'for(let i=0;i<70;i++)ST1.push({x:Math.random(),y:Math.random(),s:Math.random()*1.3+0.3});`n'
    h .= 'for(let i=0;i<40;i++)ST2.push({x:Math.random(),y:Math.random(),s:Math.random()*2+0.8});`n'
    h .= 'const PULSES=[];const nodeFlash=new Array(N).fill(0);`n'
    h .= 'let alpha=1,scale=1,tx=0,ty=0,sel=-1,hov=-1,dragN=-1,panning=false,px=0,py=0,moved=false,BG=null;`n'
    h .= 'let camT=null,cine=false,cineT=0,sim=null,pathE=null,pathN=null,pathMsg="";`n'
    h .= 'let fxMode="auto",autoLow=false,dtAvg=16,lastT=0,frame=0,idleT=0;`n'
    h .= 'function hexA(cx,a){const r=parseInt(cx.substr(1,2),16),g=parseInt(cx.substr(3,2),16),b=parseInt(cx.substr(5,2),16);return "rgba("+r+","+g+","+b+","+a+")";}`n'
    h .= 'function makeSprite(col){const c=document.createElement("canvas");c.width=64;c.height=64;const g=c.getContext("2d");const gr=g.createRadialGradient(32,32,2,32,32,31);gr.addColorStop(0,"#ffffff");gr.addColorStop(0.22,col);gr.addColorStop(0.5,col+"66");gr.addColorStop(1,col+"00");g.fillStyle=gr;g.fillRect(0,0,64,64);return c;}`n'
    h .= 'const SPR={};for(const k in CATS)SPR[k]=makeSprite(CATS[k][1]);`n'
    h .= 'function fxOn(){return fxMode==="alto"||(fxMode==="auto"&&!autoLow);}`n'
    h .= 'function reheat(){alpha=1;}`n'
    h .= 'function resize(){cv.width=innerWidth;cv.height=innerHeight;BG=ctx.createRadialGradient(cv.width*0.35,cv.height*0.25,60,cv.width*0.5,cv.height*0.5,Math.max(cv.width,cv.height)*0.75);BG.addColorStop(0,"#181826");BG.addColorStop(1,"#0a0a12");}`n'
    h .= 'addEventListener("resize",resize);resize();tx=innerWidth/2;ty=innerHeight/2;`n'
    h .= 'function radius(i){return 3.5+Math.min(11,Math.sqrt(DEG[i])*1.7);}`n'
    h .= 'function view(){let b=1;if(fxOn()&&idleT>240&&sel<0&&hov<0&&!cine&&sim===null)b=1+0.02*Math.sin(frame*0.008);const cx=innerWidth/2,cy=innerHeight/2;return{s:scale*b,x:cx-(cx-tx)*b,y:cy-(cy-ty)*b};}`n'
    h .= 'function toWorldAt(mx,my,v){return{x:(mx-v.x)/v.s,y:(my-v.y)/v.s};}`n'
    h .= 'function toWorld(mx,my){return toWorldAt(mx,my,view());}`n'
    h .= 'function recalcVisible(){for(let i=0;i<N;i++){let v=catOn[NODES[i].c]&&(!soloFlujo||REACH.has(i));`n'
    h .= 'if(focusI>=0)v=v&&(i===focusI||IN[focusI].indexOf(i)>=0||OUT[focusI].indexOf(i)>=0);`n'
    h .= 'visible[i]=v;}reheat();refreshStats();}`n'
    h .= 'function refreshStats(){let nv=0;for(let i=0;i<N;i++)if(visible[i])nv++;let ne=0;for(const e of EDGES)if(visible[e[0]]&&visible[e[1]])ne++;document.getElementById("stats").textContent=nv+" funciones - "+ne+" llamadas"+(focusI>=0?" (enfoque)":"")+(sim!==null?(sim.done?" - flujo completo":" - simulando..."):"");}`n'
    h .= 'function ctrlPt(A,B,i){const mx=(A.x+B.x)/2,my=(A.y+B.y)/2,dx=B.x-A.x,dy=B.y-A.y,d=Math.sqrt(dx*dx+dy*dy)||1,k=((i%2)?1:-1)*Math.min(30,d*0.2);return{x:mx-dy/d*k,y:my+dx/d*k};}`n'
    h .= 'function qpt(A,M,B,t){const u=1-t;return{x:u*u*A.x+2*u*t*M.x+t*t*B.x,y:u*u*A.y+2*u*t*M.y+t*t*B.y};}`n'
    h .= 'function spawnPulse(ei,fast){PULSES.push({e:ei,t:0,sp:fast?0.03:0.007+Math.random()*0.011});}`n'
    h .= 'function flyTo(i,z){const p=P[i];camT={s:z,x:innerWidth/2-p.x*z,y:innerHeight/2-p.y*z};}`n'
    h .= 'function bigBang(){for(let i=0;i<N;i++){const p=P[i],d=Math.sqrt(p.x*p.x+p.y*p.y)||1,k=16+Math.random()*26;p.vx+=p.x/d*k;p.vy+=p.y/d*k;}alpha=1;}`n'
    h .= 'function salirCine(){cine=false;document.getElementById("bCine").classList.remove("on");}`n'
    h .= 'function clearAll(){sel=-1;info.style.display="none";pathE=null;pathN=null;pathMsg="";sim=null;salirCine();if(focusI>=0){focusI=-1;recalcVisible();}else refreshStats();}`n'
    h .= 'function step(ts){`n'
    h .= 'const dt=(lastT&&ts)?Math.min(50,ts-lastT):16;lastT=ts||0;dtAvg=dtAvg*0.95+dt*0.05;frame++;idleT++;`n'
    h .= 'if(fxMode==="auto"&&dtAvg>42)autoLow=true;`n'
    h .= 'if(camT){tx+=(camT.x-tx)*0.07;ty+=(camT.y-ty)*0.07;scale+=(camT.s-scale)*0.07;if(Math.abs(camT.x-tx)<2&&Math.abs(camT.s-scale)<0.01)camT=null;}`n'
    h .= 'if(cine){cineT++;if(cineT%420===1){if(Math.random()<0.3){camT={s:0.62,x:innerWidth/2,y:innerHeight/2};select(-1,false);}else{const cand=[];for(let i=0;i<N;i++)if(visible[i]&&DEG[i]>=8)cand.push(i);if(cand.length){const i=cand[(Math.random()*cand.length)|0];sel=i;mostrarInfo(i);flyTo(i,1.5);}}}}`n'
    h .= 'if(sim!==null&&!sim.done&&frame%26===0){const next=[];for(const v of sim.front)for(const w of OUT[v]){if(sim.visited.has(w)||!visible[w])continue;sim.visited.add(w);next.push(w);nodeFlash[w]=1;const ei=EIDX.get(v+"-"+w);if(ei!==undefined)spawnPulse(ei,true);}sim.front=next;if(!next.length){sim.done=true;refreshStats();}}`n'
    h .= 'for(let i=0;i<N;i++)if(nodeFlash[i]>0)nodeFlash[i]*=0.965;`n'
    h .= 'if(alpha>0.012){`n'
    h .= 'for(let i=0;i<N;i++){if(!visible[i])continue;const a=P[i];`n'
    h .= 'for(let j=i+1;j<N;j++){if(!visible[j])continue;const b=P[j];`n'
    h .= 'let dx=a.x-b.x,dy=a.y-b.y,d2=dx*dx+dy*dy;if(d2<1)d2=1;if(d2>24000)continue;`n'
    h .= 'const f=520*alpha/d2,fx=dx*f,fy=dy*f;a.vx+=fx;a.vy+=fy;b.vx-=fx;b.vy-=fy;}}`n'
    h .= 'for(const e of EDGES){if(!visible[e[0]]||!visible[e[1]])continue;const a=P[e[0]],b=P[e[1]];`n'
    h .= 'let dx=b.x-a.x,dy=b.y-a.y;const d=Math.sqrt(dx*dx+dy*dy)||1,f=0.02*alpha*(d-64)/d,fx=dx*f,fy=dy*f;`n'
    h .= 'a.vx+=fx;a.vy+=fy;b.vx-=fx;b.vy-=fy;}`n'
    h .= 'for(let i=0;i<N;i++){if(!visible[i])continue;const p=P[i],cc=CCENT[NODES[i].c];`n'
    h .= 'p.vx+=(cc.x-p.x)*0.0022*alpha;p.vy+=(cc.y-p.y)*0.0022*alpha;`n'
    h .= 'p.vx-=p.x*0.0003*alpha;p.vy-=p.y*0.0003*alpha;`n'
    h .= 'if(i!==dragN){p.x+=p.vx;p.y+=p.vy;}p.vx*=0.6;p.vy*=0.6;}`n'
    h .= 'alpha*=0.9935;}`n'
    h .= 'const hl=sel>=0?sel:hov;`n'
    h .= 'const maxP=fxOn()?140:55;`n'
    h .= 'const intentos=(hl>=0)?4:2;`n'
    h .= 'for(let t=0;t<intentos;t++){if(PULSES.length>=maxP)break;`n'
    h .= 'const ei=(Math.random()*EDGES.length)|0,e=EDGES[ei];`n'
    h .= 'if(!visible[e[0]]||!visible[e[1]])continue;`n'
    h .= 'if(hl>=0&&e[0]!==hl&&e[1]!==hl)continue;`n'
    h .= 'if(hl<0&&Math.random()<0.45)continue;`n'
    h .= 'spawnPulse(ei,false);}`n'
    h .= 'if(pathE!==null)for(const ei of pathE)if(Math.random()<0.10&&PULSES.length<maxP+30)spawnPulse(ei,false);`n'
    h .= 'for(let i=PULSES.length-1;i>=0;i--){const p=PULSES[i];p.t+=p.sp;const e=EDGES[p.e];`n'
    h .= 'if(p.t>=1||!visible[e[0]]||!visible[e[1]])PULSES.splice(i,1);}`n'
    h .= 'draw(hl);if(frame%2===0)drawMM();requestAnimationFrame(step);}`n'
    h .= 'function stars(arr,par,drift){ctx.fillStyle="#fff";for(const s of arr){const xx=((s.x+tx*par/innerWidth)%1+1)%1*cv.width,yy=((s.y+ty*par/innerHeight+frame*drift)%1+1)%1*cv.height;ctx.globalAlpha=0.06+s.s*0.06;ctx.fillRect(xx,yy,s.s,s.s);}ctx.globalAlpha=1;}`n'
    h .= 'function draw(hl){const v=view(),vs=v.s;`n'
    h .= 'ctx.setTransform(1,0,0,1,0,0);ctx.fillStyle=BG;ctx.fillRect(0,0,cv.width,cv.height);`n'
    h .= 'if(fxOn()){stars(ST1,0.012,0.00002);stars(ST2,0.035,0.00005);}`n'
    h .= 'ctx.setTransform(vs,0,0,vs,v.x,v.y);`n'
    h .= 'let nb=null;if(hl>=0){nb=new Set([hl]);for(const w of OUT[hl])nb.add(w);for(const w of IN[hl])nb.add(w);}`n'
    h .= 'if(nb===null&&sim===null){ctx.textAlign="center";ctx.font="bold "+(30/vs)+"px Segoe UI";`n'
    h .= 'for(const k in CATS){let sx=0,sy=0,c=0;`n'
    h .= 'for(let i=0;i<N;i++){if(!visible[i]||NODES[i].c!==k)continue;sx+=P[i].x;sy+=P[i].y;c++;}`n'
    h .= 'if(c<4)continue;ctx.globalAlpha=0.10;ctx.fillStyle=CATS[k][1];ctx.fillText(CATS[k][0],sx/c,sy/c);}`n'
    h .= 'ctx.globalAlpha=1;ctx.textAlign="left";}`n'
    h .= 'for(let ei=0;ei<EDGES.length;ei++){const e=EDGES[ei],a=e[0],b=e[1];if(!visible[a]||!visible[b])continue;`n'
    h .= 'const enPath=pathE!==null&&pathE.has(ei);`n'
    h .= 'const on=nb!==null&&(a===hl||b===hl);`n'
    h .= 'const dimE=(nb!==null&&!on&&!enPath)||(sim!==null&&!(sim.visited.has(a)&&sim.visited.has(b)));`n'
    h .= 'if(enPath){ctx.strokeStyle="rgba(255,213,74,0.95)";ctx.lineWidth=2.4/vs;}`n'
    h .= 'else if(dimE){ctx.strokeStyle="rgba(255,255,255,0.02)";ctx.lineWidth=0.7/vs;}`n'
    h .= 'else if(on){ctx.strokeStyle=(a===hl)?"rgba(110,230,140,0.95)":"rgba(120,175,255,0.95)";ctx.lineWidth=1.7/vs;}`n'
    h .= 'else{ctx.strokeStyle=hexA(CATS[NODES[a].c][1],0.10);ctx.lineWidth=0.7/vs;}`n'
    h .= 'const A=P[a],B=P[b],M=ctrlPt(A,B,ei);`n'
    h .= 'ctx.beginPath();ctx.moveTo(A.x,A.y);ctx.quadraticCurveTo(M.x,M.y,B.x,B.y);ctx.stroke();`n'
    h .= 'if(on||enPath){const dx=B.x-M.x,dy=B.y-M.y,d=Math.sqrt(dx*dx+dy*dy)||1,ux=dx/d,uy=dy/d;`n'
    h .= 'const ex=B.x-ux*(radius(b)+3),ey=B.y-uy*(radius(b)+3),s=5.5/vs;`n'
    ...
    h .= 'ctx.globalCompositeOperation="lighter";`n'
    h .= 'for(const p of PULSES){const e=EDGES[p.e],A=P[e[0]],B=P[e[1]],M=ctrlPt(A,B,p.e),q=qpt(A,M,B,p.t);`n'
    h .= 'let col;if(pathE!==null&&pathE.has(p.e))col="#ffd54a";else if(nb!==null&&e[0]===hl)col="#6ee68c";else if(nb!==null&&e[1]===hl)col="#78afff";else col=CATS[NODES[e[0]].c][1];`n'
    h .= 'const dimP=(nb!==null&&e[0]!==hl&&e[1]!==hl&&!(pathE!==null&&pathE.has(p.e)));`n'
    h .= 'ctx.globalAlpha=dimP?0.04:0.9;`n'
    h .= 'if(fxOn()){const qa=qpt(A,M,B,Math.max(0,p.t-0.08));ctx.strokeStyle=col;ctx.lineWidth=1.4/vs;ctx.beginPath();ctx.moveTo(qa.x,qa.y);ctx.lineTo(q.x,q.y);ctx.stroke();}`n'
    h .= 'ctx.fillStyle=col;ctx.beginPath();ctx.arc(q.x,q.y,2/Math.sqrt(vs),0,6.2832);ctx.fill();}`n'
    h .= 'ctx.globalAlpha=1;`n'
    h .= 'for(let i=0;i<N;i++){if(!visible[i])continue;const p=P[i];let r=radius(i);`n'
    h .= 'if(fxOn()&&DEG[i]>=18)r*=1+0.06*Math.sin(frame*0.03+i);`n'
    h .= 'const dimN=(nb!==null&&!nb.has(i))||(sim!==null&&!sim.visited.has(i));`n'
    h .= 'ctx.globalAlpha=dimN?0.06:1;`n'
    h .= 'const sz=r*4.6;ctx.drawImage(SPR[NODES[i].c],p.x-sz/2,p.y-sz/2,sz,sz);}`n'
    h .= 'ctx.globalAlpha=1;ctx.globalCompositeOperation="source-over";`n'
    h .= 'for(let i=0;i<N;i++){if(!visible[i])continue;const p=P[i],r=radius(i);`n'
    h .= 'if(i===sel||i===hov){ctx.strokeStyle="#fff";ctx.globalAlpha=0.9;ctx.lineWidth=1.6/vs;ctx.beginPath();ctx.arc(p.x,p.y,r+2/vs,0,6.2832);ctx.stroke();}`n'
    h .= 'if(nodeFlash[i]>0.03){ctx.strokeStyle="#fff";ctx.globalAlpha=nodeFlash[i]*0.75;ctx.lineWidth=1.4/vs;ctx.beginPath();ctx.arc(p.x,p.y,r+(1-nodeFlash[i])*30,0,6.2832);ctx.stroke();}}`n'
    h .= 'ctx.globalAlpha=1;`n'
    h .= 'ctx.font=(11/vs)+"px Segoe UI";`n'
    h .= 'for(let i=0;i<N;i++){if(!visible[i])continue;`n'
    h .= 'const enP=pathN!==null&&pathN.has(i);`n'
    h .= 'if(sim!==null){if(!sim.visited.has(i))continue;if(!(enP||i===hov||i===sel||vs>1.5||DEG[i]>=14))continue;}`n'
    h .= 'else if(nb!==null){if(!nb.has(i))continue;}`n'
    h .= 'else if(!(enP||i===hov||vs>1.5||(DEG[i]>=14&&vs>0.55)))continue;`n'
    h .= 'const p=P[i];ctx.fillStyle="rgba(0,0,0,0.55)";ctx.fillText(NODES[i].n,p.x+radius(i)+3/vs,p.y+4.5/vs);`n'
    h .= 'ctx.fillStyle="#eef";ctx.fillText(NODES[i].n,p.x+radius(i)+2/vs,p.y+3.5/vs);}}`n'
    h .= 'function drawMM(){const W=mm.width,H=mm.height;mtx.clearRect(0,0,W,H);`n'
    h .= 'let mnx=1e9,mny=1e9,mxx=-1e9,mxy=-1e9;`n'
    h .= 'for(let i=0;i<N;i++){if(!visible[i])continue;const p=P[i];if(p.x<mnx)mnx=p.x;if(p.x>mxx)mxx=p.x;if(p.y<mny)mny=p.y;if(p.y>mxy)mxy=p.y;}`n'
    h .= 'if(mnx>mxx)return;const pad=30;mnx-=pad;mny-=pad;mxx+=pad;mxy+=pad;`n'
    h .= 'const s=Math.min(W/(mxx-mnx),H/(mxy-mny)),ox=(W-(mxx-mnx)*s)/2,oy=(H-(mxy-mny)*s)/2;`n'
    h .= 'mm._c={mnx:mnx,mny:mny,s:s,ox:ox,oy:oy};`n'
    h .= 'for(let i=0;i<N;i++){if(!visible[i])continue;const p=P[i];mtx.fillStyle=CATS[NODES[i].c][1];mtx.globalAlpha=0.85;mtx.fillRect(ox+(p.x-mnx)*s-1,oy+(p.y-mny)*s-1,2,2);}`n'
    h .= 'mtx.globalAlpha=1;const v=view();`n'
    h .= 'const c0=toWorldAt(0,0,v),c1=toWorldAt(innerWidth,0,v),c2=toWorldAt(innerWidth,innerHeight,v),c3=toWorldAt(0,innerHeight,v);`n'
    h .= 'mtx.strokeStyle="rgba(255,255,255,0.7)";mtx.lineWidth=1;mtx.beginPath();`n'
    h .= 'mtx.moveTo(ox+(c0.x-mnx)*s,oy+(c0.y-mny)*s);mtx.lineTo(ox+(c1.x-mnx)*s,oy+(c1.y-mny)*s);mtx.lineTo(ox+(c2.x-mnx)*s,oy+(c2.y-mny)*s);mtx.lineTo(ox+(c3.x-mnx)*s,oy+(c3.y-mny)*s);mtx.closePath();mtx.stroke();}`n'
    h .= 'mm.addEventListener("mousedown",e=>{const c=mm._c;if(!c)return;const r=mm.getBoundingClientRect(),x=(e.clientX-r.left-c.ox)/c.s+c.mnx,y=(e.clientY-r.top-c.oy)/c.s+c.mny;camT={s:scale,x:innerWidth/2-x*scale,y:innerHeight/2-y*scale};});`n'
    ...
    h .= 'cv.addEventListener("mousedown",e=>{idleT=0;salirCine();px=e.clientX;py=e.clientY;moved=false;const t=hit(px,py);if(t>=0)dragN=t;else panning=true;cv.style.cursor="grabbing";});`n'
    h .= 'addEventListener("mousemove",e=>{idleT=0;const mx=e.clientX,my=e.clientY;`n'
    ...
    h .= 'addEventListener("mouseup",e=>{if(!moved&&e.target===cv){const t=hit(e.clientX,e.clientY);if(t>=0)select(t,false);}dragN=-1;panning=false;cv.style.cursor="grab";});`n'
    h .= 'cv.addEventListener("dblclick",()=>select(-1,false));`n'
    h .= 'addEventListener("keydown",e=>{idleT=0;if(e.key==="Escape")clearAll();else if(e.key===" "&&e.target.tagName!=="INPUT"){e.preventDefault();bigBang();}});`n'
    h .= 'cv.addEventListener("wheel",e=>{e.preventDefault();idleT=0;salirCine();camT=null;const f=e.deltaY<0?1.15:0.87,mx=e.clientX,my=e.clientY;tx=mx-(mx-tx)*f;ty=my-(my-ty)*f;scale*=f;},{passive:false});`n'
    h .= 'const info=document.getElementById("info");`n'
    h .= 'function linkA(i){return "<a data-i=\"" + i + "\">" + NODES[i].n + "</a>";}`n'
    h .= 'function mostrarInfo(i){const out=OUT[i],inn=IN[i];`n'
    h .= 'const fTxt=(focusI===i)?"Salir del enfoque":"Ver solo su red";`n'
    h .= 'let hx="<h3><span class=\"dot\" style=\"background:"+CATS[NODES[i].c][1]+";display:inline-block;margin-right:6px\"></span>"+NODES[i].n+"</h3>";`n'
    h .= 'hx+="<div class=\"mut\">"+CATS[NODES[i].c][0]+" - "+DEG[i]+" conexiones - alcanza "+RCNT[i]+" funciones</div>";`n'
    h .= 'hx+="<div class=\"mut\">"+(DEPTH[i]>=1e9?"Iniciar no llega aqui por llamadas":"A "+DEPTH[i]+" salto"+(DEPTH[i]===1?"":"s")+" de Iniciar")+"</div>";`n'
    h .= 'if(pathMsg)hx+="<div class=\"mut\" style=\"color:#ffd750\">"+pathMsg+"</div>";`n'
    h .= 'hx+="<p><a data-focus=\""+i+"\">"+fTxt+"</a><a data-path=\""+i+"\">Camino desde Iniciar</a><a data-sim=\""+i+"\">Simular flujo desde aqui</a></p>";`n'
    h .= 'hx+="<p><b>Llama a ("+out.length+"):</b></p>"+(out.map(linkA).join("")||"<span class=\"mut\">nada</span>");`n'
    h .= 'hx+="<p><b>Llamado por ("+inn.length+"):</b></p>"+(inn.map(linkA).join("")||"<span class=\"mut\">nadie</span>");`n'
    h .= 'info.innerHTML=hx;info.style.display="block";}`n'
    h .= 'function select(i,centrar){sel=i;if(i<0){info.style.display="none";return;}`n'
    h .= 'mostrarInfo(i);if(centrar)flyTo(i,Math.max(scale,1.15));}`n'
    h .= 'function toggleFocus(i){focusI=(focusI===i)?-1:i;recalcVisible();if(sel>=0)mostrarInfo(sel);}`n'
    h .= 'function caminoDesde(i){pathMsg="";pathE=null;pathN=null;`n'
    h .= 'if(ROOT<0){pathMsg="No existe la funcion Iniciar";mostrarInfo(i);return;}`n'
    h .= 'if(i===ROOT){pathE=new Set();pathN=new Set([ROOT]);pathMsg="Esta ES la funcion Iniciar";mostrarInfo(i);return;}`n'
    h .= 'const prev=new Array(N).fill(-1),seen=new Array(N).fill(false),q=[ROOT];let qi=0;seen[ROOT]=true;`n'
    h .= 'while(qi<q.length){const v=q[qi++];if(v===i)break;for(const w of OUT[v])if(!seen[w]){seen[w]=true;prev[w]=v;q.push(w);}}`n'
    h .= 'if(!seen[i]){pathMsg="Iniciar no llega a esta funcion por llamadas";mostrarInfo(i);return;}`n'
    h .= 'pathE=new Set();pathN=new Set([ROOT]);let v=i;`n'
    h .= 'while(v!==ROOT){pathN.add(v);const u=prev[v],ei=EIDX.get(u+"-"+v);if(ei!==undefined)pathE.add(ei);v=u;}`n'
    h .= 'pathMsg="Camino de "+pathN.size+" funciones desde Iniciar";mostrarInfo(i);}`n'
    h .= 'function startSim(root){nodeFlash.fill(0);sim={visited:new Set([root]),front:[root],done:false};nodeFlash[root]=1;pathE=null;pathN=null;pathMsg="";refreshStats();}`n'
    h .= 'document.addEventListener("click",e=>{const t=e.target.closest("a[data-i]");if(t){select(parseInt(t.getAttribute("data-i"),10),true);return;}`n'
    h .= 'const f=e.target.closest("a[data-focus]");if(f){toggleFocus(parseInt(f.getAttribute("data-focus"),10));return;}`n'
    h .= 'const pt=e.target.closest("a[data-path]");if(pt){caminoDesde(parseInt(pt.getAttribute("data-path"),10));return;}`n'
    h .= 'const sm=e.target.closest("a[data-sim]");if(sm)startSim(parseInt(sm.getAttribute("data-sim"),10));});`n'
    ...
    h .= 'document.getElementById("bSim").addEventListener("click",()=>startSim(sel>=0?sel:(ROOT>=0?ROOT:0)));`n'
    h .= 'document.getElementById("bCine").addEventListener("click",()=>{cine=!cine;cineT=0;document.getElementById("bCine").classList.toggle("on",cine);});`n'
    h .= 'document.getElementById("bBang").addEventListener("click",bigBang);`n'
    h .= 'document.getElementById("bPng").addEventListener("click",()=>{const a=document.createElement("a");a.download="grafo_macro.png";a.href=cv.toDataURL("image/png");a.click();});`n'
    h .= 'document.getElementById("bFull").addEventListener("click",()=>{if(document.fullscreenElement)document.exitFullscreen();else document.documentElement.requestFullscreen();});`n'
    h .= 'document.getElementById("bFx").addEventListener("click",()=>{fxMode=fxMode==="auto"?"alto":(fxMode==="alto"?"bajo":"auto");autoLow=false;document.getElementById("bFx").textContent="FX: "+(fxMode==="auto"?"Auto":(fxMode==="alto"?"Alto":"Bajo"));});`n'
    ...
    h .= 'const ordenDeg=[];for(let i=0;i<N;i++)ordenDeg.push(i);ordenDeg.sort((a,b)=>DEG[b]-DEG[a]);`n'
    h .= 'let topsHtml="";for(let k=0;k<8&&k<N;k++){const i=ordenDeg[k];`n'
    h .= 'topsHtml+="<a data-i=\""+i+"\"><span class=\"dot\" style=\"background:"+CATS[NODES[i].c][1]+"\"></span>"+NODES[i].n+" <span class=\"mut\">("+DEG[i]+")</span></a>";}`n'
    h .= 'document.getElementById("tops").innerHTML=topsHtml;`n'
    h .= '{let mIn=0,mOut=0,mR=0,deepI=-1,deepD=-1;`n'
    h .= 'for(let i=0;i<N;i++){if(IN[i].length>IN[mIn].length)mIn=i;if(OUT[i].length>OUT[mOut].length)mOut=i;if(RCNT[i]>RCNT[mR])mR=i;if(DEPTH[i]<1e9&&DEPTH[i]>deepD){deepD=DEPTH[i];deepI=i;}}`n'
    h .= 'const dead=[];for(let i=0;i<N;i++)if(IN[i].length===0&&SEEDS.indexOf(i)<0)dead.push(i);`n'
    h .= 'dead.sort((a,b)=>NODES[a].n.localeCompare(NODES[b].n));`n'
    h .= 'let ins="<p>La mas llamada: "+linkA(mIn)+" ("+IN[mIn].length+" llamadores)</p>";`n'
    h .= 'ins+="<p>La que mas llama: "+linkA(mOut)+" (llama a "+OUT[mOut].length+")</p>";`n'
    h .= 'ins+="<p>La mas influyente: "+linkA(mR)+" (alcanza "+RCNT[mR]+" funciones)</p>";`n'
    h .= 'if(deepI>=0)ins+="<p>La mas profunda: "+linkA(deepI)+" (a "+deepD+" saltos de Iniciar)</p>";`n'
    h .= 'ins+="<p>Posiblemente sin uso ("+dead.length+"): "+dead.slice(0,12).map(linkA).join(", ")+(dead.length>12?" ...":"")+"</p>";`n'
    h .= 'document.getElementById("insBody").innerHTML=ins;}`n'
