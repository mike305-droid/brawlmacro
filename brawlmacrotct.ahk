...
ParchesPaginas() {
    return [
    { ico: Chr(0x1F4CB), tit: "Parche v32.5.1 (actual)",
...
GrafoConstruirHtml(nodesJson, edgesJson, seedsJson) {
    ...
    h .= '#ui{position:fixed;top:10px;left:10px;background:rgba(14,14,26,.92);border:1px solid #34344a;border-radius:12px;padding:10px 12px;width:252px;backdrop-filter:blur(4px)}`n'
    h .= '#info{position:fixed;top:10px;right:10px;width:290px;max-height:92vh;overflow:auto;background:rgba(14,14,26,.92);border:1px solid #34344a;border-radius:12px;padding:10px 12px;display:none;backdrop-filter:blur(4px)}`n'
    ...
    h .= '<div class="mut" style="margin-top:6px">Rueda: zoom - Arrastrar fondo: mover - Clic en nodo: detalles - Doble clic o Esc: soltar</div></div>`n'
    h .= '<div id="info"></div>`n'
    h .= '<script>`n'
    ...
    h .= 'const STARS=[];for(let i=0;i<90;i++)STARS.push({x:Math.random(),y:Math.random(),s:Math.random()*1.4+0.3,v:0.00008+Math.random()*0.00025});`n'
    h .= 'const PULSES=[];`n'
    ...
    h .= 'function refreshStats(){let nv=0;for(let i=0;i<N;i++)if(visible[i])nv++;let ne=0;for(const e of EDGES)if(visible[e[0]]&&visible[e[1]])ne++;document.getElementById("stats").textContent=nv+" funciones - "+ne+" llamadas"+(focusI>=0?" (enfoque)":"");}`n'
    ...
    h .= 'for(let t=0;t<intentos;t++){if(PULSES.length>=130)break;`n'
    ...
    h .= 'PULSES.push({e:ei,t:Math.random()*0.15,sp:0.007+Math.random()*0.011});}`n'
    ...
    h .= 'draw(hl);requestAnimationFrame(step);}`n'
    h .= 'function draw(hl){`n'
    h .= 'ctx.setTransform(1,0,0,1,0,0);ctx.fillStyle=BG;ctx.fillRect(0,0,cv.width,cv.height);`n'
    h .= 'ctx.fillStyle="#ffffff";`n'
    h .= 'for(const s of STARS){s.y+=s.v;if(s.y>1)s.y-=1;ctx.globalAlpha=0.08+s.s*0.07;ctx.fillRect(s.x*cv.width,s.y*cv.height,s.s,s.s);}`n'
    h .= 'ctx.globalAlpha=1;`n'
    h .= 'ctx.setTransform(scale,0,0,scale,tx,ty);`n'
    h .= 'let nb=null;if(hl>=0){nb=new Set([hl]);for(const v of OUT[hl])nb.add(v);for(const v of IN[hl])nb.add(v);}`n'
    h .= 'if(nb===null){ctx.textAlign="center";ctx.font="bold "+(30/scale)+"px Segoe UI";`n'
    ...
    h .= 'if(nb!==null&&!on)ctx.strokeStyle="rgba(255,255,255,0.02)";`n'
    h .= 'else if(on)ctx.strokeStyle=(a===hl)?"rgba(110,230,140,0.95)":"rgba(120,175,255,0.95)";`n'
    h .= 'else ctx.strokeStyle="rgba(140,150,220,0.10)";`n'
    h .= 'ctx.lineWidth=(on?1.7:0.7)/scale;`n'
    ...
    h .= 'if(on){const dx=B.x-M.x,dy=B.y-M.y,d=Math.sqrt(dx*dx+dy*dy)||1,ux=dx/d,uy=dy/d;`n'
    h .= 'const ex=B.x-ux*(radius(b)+3),ey=B.y-uy*(radius(b)+3),s=5.5/scale;`n'
    ...
    h .= 'let col;if(nb!==null&&e[0]===hl)col="#6ee68c";else if(nb!==null&&e[1]===hl)col="#78afff";else col=CATS[NODES[e[0]].c][1];`n'
    h .= 'ctx.globalAlpha=(nb!==null&&e[0]!==hl&&e[1]!==hl)?0.05:0.85;`n'
    h .= 'ctx.fillStyle=col;ctx.beginPath();ctx.arc(q.x,q.y,1.8/Math.sqrt(scale),0,6.2832);ctx.fill();}`n'
    h .= 'ctx.globalAlpha=1;`n'
    h .= 'for(let i=0;i<N;i++){if(!visible[i])continue;const p=P[i],r=radius(i),dim=nb!==null&&!nb.has(i);`n'
    h .= 'const col=CATS[NODES[i].c][1];`n'
    h .= 'ctx.globalAlpha=dim?0.05:0.16;ctx.fillStyle=col;`n'
    h .= 'ctx.beginPath();ctx.arc(p.x,p.y,r*2.1,0,6.2832);ctx.fill();`n'
    h .= 'ctx.globalAlpha=dim?0.14:1;`n'
    h .= 'ctx.beginPath();ctx.arc(p.x,p.y,r,0,6.2832);ctx.fill();`n'
    h .= 'if(i===sel||i===hov){ctx.strokeStyle="#fff";ctx.lineWidth=1.8/scale;ctx.globalAlpha=0.9;ctx.stroke();}`n'
    h .= 'ctx.globalAlpha=1;}`n'
    h .= 'ctx.font=(11/scale)+"px Segoe UI";`n'
    ...
    h .= 'const p=P[i];ctx.fillStyle="rgba(0,0,0,0.55)";ctx.fillText(NODES[i].n,p.x+radius(i)+3/scale,p.y+4.5/scale);`n'
    h .= 'ctx.fillStyle="#eef";ctx.fillText(NODES[i].n,p.x+radius(i)+2/scale,p.y+3.5/scale);}}`n'
    ...
    h .= 'function select(i,centrar){sel=i;if(i<0){info.style.display="none";if(focusI>=0){focusI=-1;recalcVisible();}return;}`n'
    h .= 'const out=OUT[i],inn=IN[i];`n'
    h .= 'const fTxt=(focusI===i)?"Salir del modo enfoque":"Ver solo su red";`n'
    h .= 'info.innerHTML="<h3><span class=\"dot\" style=\"background:"+CATS[NODES[i].c][1]+";display:inline-block;margin-right:6px\"></span>"+NODES[i].n+"</h3>"+`n'
    h .= '"<div class=\"mut\">"+CATS[NODES[i].c][0]+" - "+DEG[i]+" conexiones</div>"+`n'
    h .= '"<p><a data-focus=\""+i+"\">"+fTxt+"</a></p>"+`n'
    ...
    h .= 'function toggleFocus(i){focusI=(focusI===i)?-1:i;recalcVisible();select(sel,false);}`n'
    ...
    h .= 'const f=e.target.closest("a[data-focus]");if(f)toggleFocus(parseInt(f.getAttribute("data-focus"),10));});`n'
